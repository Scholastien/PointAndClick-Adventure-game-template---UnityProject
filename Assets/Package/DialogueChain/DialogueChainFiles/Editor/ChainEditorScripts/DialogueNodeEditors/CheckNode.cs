...
public class CheckNode : Node
{
    ...
    public override void DrawWindow(DialogueChain chain)
    {
        ...
        if (GUILayout.Button("Quest Check"))
        {
            cEvent.chainQuestChecks.Add(new QuestCheck());
        }
        EditorGUILayout.EndHorizontal();

        if (cEvent.triggerChecks.Count > 0 
            || cEvent.itemChecks.Count > 0 
            || cEvent.chainIntChecks.Count > 0
            || cEvent.chainQuestChecks.Count > 0)
        {
            height += 20;
            ...
            if (cEvent.chainQuestChecks.Count > 0)
            {
                count++;
            }
            if (count > 1)
        ...
        {
            height += 20;
            EditorGUILayout.LabelField("Quest Checks", EditorStyles.boldLabel);

            if (cEvent.chainQuestChecks.Count > 0)
            {
                for (int i = 0; i < cEvent.chainQuestChecks.Count; i++)
                {
                    height += 20;
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField("If ", GUILayout.Width(20));

                    cEvent.chainQuestChecks[i].quest = (Quest)EditorGUILayout.ObjectField(cEvent.chainQuestChecks[i].quest, typeof(Quest), false, GUILayout.Width(150));
                    EditorGUILayout.LabelField("", GUILayout.Width(1));


                    EditorGUILayout.LabelField("is ", GUILayout.Width(20));

                    cEvent.chainQuestChecks[i].questState = (QuestState)EditorGUILayout.EnumPopup(cEvent.chainQuestChecks[i].questState, GUILayout.Width(125));
                    EditorGUILayout.LabelField(" ", GUILayout.Width(10));
                    if (GUILayout.Button("-", GUILayout.Width(25)))
                    {
                        cEvent.chainQuestChecks.RemoveAt(i);
                        return;
                    }
