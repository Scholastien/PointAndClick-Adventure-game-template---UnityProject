...
public abstract class Node : Editor
{
    ...
    public virtual void DrawWindow(DialogueChain dialogueChain)
    {
        if (this != null)
        ...
        HandleRank();
        HandleTransition(dialogueChain);
    ...
    public virtual void HandleTransition(DialogueChain dialogueChain)
    {
        // If it's starting or ending the dialogueChain
        if(dialogueChain.startEvent  == cEvent || nextNodes.Count == 0)
        {
            height += 20;
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.BeginVertical();
            
            cEvent.transitionType = (TransitionType)EditorGUILayout.EnumPopup("Transition type: ", cEvent.transitionType);
