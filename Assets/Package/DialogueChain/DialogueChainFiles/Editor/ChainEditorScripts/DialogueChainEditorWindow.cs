...
public class DialogueChainEditorWindow : EditorWindow
{
    ...
    public void LoadCurrentQuest()
    {
        ...
        foreach (ChainEvent qEvent in dialogueChain.chainEvents)
        {
            ...
            else if (qEvent.cEventType == ChainEventType.VariableModifier)
            {
                VariableModifierNode newNode = CreateInstance("VariableModifierNode") as VariableModifierNode;
                AssignEventToNode(newNode, qEvent);
            }

            else if (qEvent.cEventType == ChainEventType.ScheduledActivity)
            {
                ScheduledActivityNode newNode = CreateInstance("ScheduledActivityNode") as ScheduledActivityNode;

                Debug.Log("Instance newnode is ScheduleActivity " + newNode == null);

                AssignEventToNode(newNode, qEvent);
            }
            else if (qEvent.cEventType == ChainEventType.MinigameLauncher)
            {
                MinigameLauncherNode newNode = CreateInstance("MinigameLauncherNode") as MinigameLauncherNode;
                
                AssignEventToNode(newNode, qEvent);
            }
            else if (qEvent.cEventType == ChainEventType.SetLocation)
            {
                LocationNode newNode = CreateInstance("LocationNode") as LocationNode;
    ...
    /// <summary>
    /// Loop in the nodes and search for each of code chain that use video
    /// for each node chain, search for the first one
    /// for each of its children (that have useVideo checked if dialogue node or other type of node)
    /// modify the bool VideoChained property they have
    /// for each of the other, set the videoChained property to false
    /// </summary>
    #region VideoSupport
    public void VideoPropagation()
    {
        //public List<Node> nodes = new List<Node>();
        //public Node startingNode;
        ...
        if(startingNode != null)
            UpdateVideoChains(startingNode, false);


    }

    public void UpdateVideoChains(Node nodeToStart, bool _inVideoChain)
    {
        bool inVideoChain = _inVideoChain;
        nodeToStart.inVideoChain = inVideoChain;
        nodeToStart.cEvent.inVideoChain = inVideoChain;
        nodeToStart.cEvent.isEnd = !(nodeToStart.nextNodes.Count > 0);
        if (!nodeToStart.cEvent.isEnd)
        {
            if (inVideoChain)
            {
                inVideoChain = (nodeToStart.GetType() != typeof(DialogueNode) || nodeToStart.cEvent.useVideo); 
            }
            else
            {
                inVideoChain = (nodeToStart.GetType() == typeof(DialogueNode) && nodeToStart.cEvent.useVideo);
            }
            foreach (Node nextNode in nodeToStart.nextNodes)
            {
                UpdateVideoChains(nextNode, inVideoChain);
            }
        }
    } 
    ...
    private void RightClickEmpty()
    {
        ...
        else
        {
            ...
            //menu.AddItem(new GUIContent("Set/Variable Adjustment"), false, CallBack, "varAdjust");
            menu.AddItem(new GUIContent("Set/ScheduleActivity Set"), false, CallBack, "setScheduleActivity");
            ...
            menu.AddItem(new GUIContent("Scene/Minigame launcher"), false, CallBack, "minigameLauncher");
            menu.AddItem(new GUIContent("Scene/Set new Location"), false, CallBack, "setLocation");
            ...
            menu.AddItem(new GUIContent("Add Trigger Check"), false, CallBack, "check");
        }
    ...
    private void RightClickNode()
    {
        ...
        if (!(selectedNode is UserInputNode || selectedNode is SecondaryInputNode || selectedNode is MinigameLauncherNode))
        {
    ...
    public void CallBack(object obj)
    {
        ...
        //else if (callBackString == "varAdjust")
        //{
        //    VariableModifierNode newNode = CreateInstance("VariableModifierNode") as VariableModifierNode;
        //    MakeNewNode(newNode, ChainEventType.VariableModifier);
        //}
        else if (callBackString == "setScheduleActivity")
        {
            ScheduledActivityNode newNode = CreateInstance("ScheduledActivityNode") as ScheduledActivityNode;
            MakeNewNode(newNode, ChainEventType.ScheduledActivity);
        }
        ...
        else if(callBackString == "minigameLauncher")
        {
            // TODO: Create a new Minigame node
            MinigameLauncherNode newNode = CreateInstance("MinigameLauncherNode") as MinigameLauncherNode;

            MakeNewNode(newNode, ChainEventType.MinigameLauncher);
        }
        else if(callBackString == "setLocation")
        {
            LocationNode newNode = CreateInstance("LocationNode") as LocationNode;

            MakeNewNode(newNode, ChainEventType.SetLocation);
        }
