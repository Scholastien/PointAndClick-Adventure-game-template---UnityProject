    ...
    public override void OnInspectorGUI()
    {
        ...
        if (editor.showSpeakerName)
        {
            ...
            if (speakersArray.Length > 0 && !hideList)
            {
                ...
                {
                    speakerIndex = System.Array.IndexOf(Enum.GetNames(typeof(CharacterFunction)), editor.speaker);
                }
                speakerIndex = EditorGUILayout.Popup(speakerIndex, speakersArray);
                editor.speaker = Enum.GetNames(typeof(CharacterFunction))[speakerIndex];
            ...
            else
            {
                ...
                if (Enum.GetNames(typeof(CharacterFunction)).Length > 0)
                {
                    if (GUILayout.Button("List"))
                    {
                        editor.speaker = Enum.GetNames(typeof(CharacterFunction))[0];
                        hideList = false;
                    }
                }
            }
